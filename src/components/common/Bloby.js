...
const Bloby = ({parentColor, width, height, blobRadius, lineWidth}) => {
    ...
    function blob(blobObject) {
        ...
        if (document.getElementById(blobObject.id)) {
            ID = blobObject.id + 'myCanvas-' + uuid();
        } else {
            ID = blobObject.id || 'myCanvas-' + uuid();
        }
        var width = blobObject.width;
        var height = blobObject.height;
        var nodeCount = blobObject.nodeCount || 10;
        var origin = blobObject.origin || [(width) / 2, (height) / 2];
        var circlePositions = [];
        ...
        if (blobObject.parent) {
            blobObject.parent.appendChild(createdElement);
        ...
        function animate(timestamp) {
            ...
            ((function () { var x = { 0: function () { if (blobObject.bounce>0) window.requestAnimationFrame(animate) }, 1: function () { } }; return x[isStatic] })())();
        }
    ...
    const addDivRef = React.useCallback(
        (divGraph) => {
            try {
                ...
                divGraph.innerHTML = '';
                blob({
                    nodeCount: rand(3, 8), width: width, height: height, origin: [rand(30, 250), rand(30, 250)], blobRadius: blobRadius, lineWidth: lineWidth, strokeStyle: "white", fillStyle: parentColor,
                    randomness: rand(3, 8), fillOpacity: 1, lineOpacity: 1, minPeriod: 2, maxPeriod: 5, parent: divGraph,
                    harmonicRadius: rand(10, 16), attraction: 0.5, bounce: 0, xray: 0
                });
    ...
    return (
        <div style={{overflow: 'hidden'}} ref={addDivRef}>This project is empty</div>
    )
}
